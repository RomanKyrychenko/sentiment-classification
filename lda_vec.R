...
train_lda <- function(tem,k){
  ...
  doc_topic_distr = 
    lda_model$fit_transform(x = dtm_train, n_iter = 500, 
...
out_df <- as_tibble(do.call("smartbind",lapply(out,function(x) as.data.frame(x,stringsAsFactors=F))))
...
Sys.sleep(100)
res <- tryCatch(docs_bulk(paste0("OU",result_date,".json")))
